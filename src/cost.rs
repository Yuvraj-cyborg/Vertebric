...
impl ModelPricing {
    fn lookup(model: &str) -> Self {
        ...
        if m.contains("opus-4-6") || m.contains("opus-4-5") {
            Self { input: 5.0, output: 25.0, cache_read: 0.5, cache_write: 6.25 }
        } else if m.contains("opus") {
            Self { input: 15.0, output: 75.0, cache_read: 1.5, cache_write: 18.75 }
        } else if m.contains("sonnet") {
            Self { input: 3.0, output: 15.0, cache_read: 0.3, cache_write: 3.75 }
        } else if m.contains("haiku-4") {
            Self { input: 1.0, output: 5.0, cache_read: 0.1, cache_write: 1.25 }
        } else if m.contains("haiku") {
            Self { input: 0.8, output: 4.0, cache_read: 0.08, cache_write: 1.0 }
        } else if m.contains("gpt-4o-mini") {
            Self { input: 0.15, output: 0.6, cache_read: 0.075, cache_write: 0.15 }
        } else if m.contains("gpt-4o") || m.contains("gpt-4-turbo") {
            Self { input: 2.5, output: 10.0, cache_read: 1.25, cache_write: 2.5 }
        } else if m.contains("gpt-4") {
            Self { input: 30.0, output: 60.0, cache_read: 15.0, cache_write: 30.0 }
        } else if m.contains("o1") || m.contains("o3") || m.contains("o4-mini") {
            Self { input: 3.0, output: 12.0, cache_read: 1.5, cache_write: 3.0 }
        } else if m.contains("gemini-2.5-pro") {
            Self { input: 1.25, output: 10.0, cache_read: 0.31, cache_write: 1.25 }
        } else if m.contains("gemini-2.5-flash") || m.contains("gemini-2.0-flash") {
            Self { input: 0.15, output: 0.6, cache_read: 0.037, cache_write: 0.15 }
